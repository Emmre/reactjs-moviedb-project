...
class App extends Component {
  ...
  handleSubmit = (e) => { 
    fetch(`https://api.themoviedb.org/3/search/movie?api_key=f88a5a8b79ac4e57c9eef7bfd5411d82&query=${this.state.searchTerm}&language=en-US&page=${this.state.currentPage}`)
    .then(data => data.json())
    .then(data => {
  ...
  nextPage = (pageNumber) => {
    fetch(`https://api.themoviedb.org/3/search/movie?api_key=f88a5a8b79ac4e57c9eef7bfd5411d82&query=${this.state.searchTerm}&language=en-US&page=${pageNumber}`)
    .then(data => data.json())
    .then(data => {
